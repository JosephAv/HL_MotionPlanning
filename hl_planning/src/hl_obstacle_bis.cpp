...
Eigen::RowVectorXd t_finale(2*NUM_COLS-1);
Eigen::MatrixXd final_trajectory(6, 2*NUM_COLS-1);
double final_t_vp;
...
void obs_traj_computation(Eigen::VectorXd inizio, Eigen::VectorXd fine, Eigen::VectorXd vel_inizio, Eigen::VectorXd vel_fine, double t_iniziale, double t_fine, Eigen::MatrixXd ostacoli)
{
    ...
    Eigen::MatrixXd candidate_trajectory(3, 2*NUM_COLS-1);
    ...
    Eigen::RowVectorXd t_aux(NUM_COLS);
    ...
    for(int n=0; n<2000; n++)
    {
        ...
        candidate_trajectory.block(0, NUM_COLS, 3, NUM_COLS-1) = traj_aux.block(0,1,3,NUM_COLS-1);

        l_candidate = trajectory_length_computation(candidate_trajectory);

        if (l_candidate < l_opt)
        {
            final_trajectory.block(0, 0, 3, 2*NUM_COLS-1) = candidate_trajectory;
            l_opt = l_candidate;
            //t_finale.head(NUM_COLS) = Eigen::RowVectorXd::LinSpaced(NUM_COLS, t_iniziale, t_vp);
            //t_aux = Eigen::RowVectorXd::LinSpaced(NUM_COLS, t_vp, t_fine);
            //t_finale.tail(NUM_COLS-1) = t_aux.tail(NUM_COLS-1);
            final_t_vp = t_vp;
        }
    ...
    for (int i = 0; i < 3; i++)
    {
        traj_aux.block(i,0,1,NUM_COLS) =  single_dof(vp_ori(i), fine_ori(i), v_vp_ori(i), vel_fine(i+3), i, ((t_fine - final_t_vp)/(NUM_COLS-1)));
    }

    final_trajectory.block(3,NUM_COLS,3,NUM_COLS-1) = traj_aux.block(0,1,3,NUM_COLS-1);
